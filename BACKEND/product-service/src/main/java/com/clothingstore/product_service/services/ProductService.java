...
public class ProductService {
    ...
    public ProductDTO deleteProduct(Integer id) {
        Optional<Product> productEntityOpt = productRepository.findById(id);
        Product toDeleteProduct = productEntityOpt.orElseThrow(() -> new RuntimeException("Product not found"));
        productVariantService.deleteAllByProductId(id);
        productRepository.delete(toDeleteProduct);
        return productMapper.toDTO(toDeleteProduct);
    }
}
